...
class TestExtensionVisitor(object):
    def test_analyser(self, tmp_path: Path):
        (tmp_path / 'test.f90').write_text('Test file')
        (tmp_path / 'directory').mkdir()
        (tmp_path / 'directory' / 'another.f90').write_text('Test file too')
        ...
        test_unit = ExtensionVisitor(emap, fmap, db, tmp_path)
        tracker.clear()
        test_unit.visit(tmp_path / 'test.f90')
        assert tracker == [tmp_path / 'test.f90']

        test_unit.visit(tmp_path / 'directory' / 'another.f90')
        assert tracker == [tmp_path / 'test.f90',
                           tmp_path / 'directory/another.f90']
