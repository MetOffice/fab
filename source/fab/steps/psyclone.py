...
@step
def psyclone(
        config, kernel_roots: Optional[List[Path]] = None,
        transformation_script: Optional[Callable[[Path, BuildConfig], Path]] = None,  # noqa: E501
    ...
    mp_payload = _generate_mp_payload(
        ...
        kernel_roots, transformation_script, cli_args
    )
    ...
    #       given overrides folder is this called psykal?
    # assert False
...
def _analyse_kernels(config, kernel_roots) -> Dict[str, int]:
    ...
    .. _Psyclone built-ins: https://psyclone.readthedocs.io/en/stable/dynamo0p3.html#built-ins

    The Psyclone and Analyse steps both use the generic Fortran analyser,
    which recognises Psyclone kernel metadata. The Analysis step must come
    after this step because it needs to analyse the fortran we create.

    """  # noqa: E501
    # Ignore the prebuild folder.
    # Todo: test the prebuild folder is ignored, in case someone breaks this.
    file_lists = [list(file_walk(root,
    ...
    # metadata.
    # todo: We'd like to separate that from the general fortran analyser
    #       at some point, to reduce coupling.
    ...
    # mark the analysis results files (i.e. prebuilds) as being current, so the
    # cleanup knows not to delete them
    ...
    for af in analysed_fortran:
        assert set(af.psyclone_kernels).isdisjoint(all_kernel_hashes), \
            ...
            f"{set(af.psyclone_kernels) & set(all_kernel_hashes)}"
...
def do_one_file(arg: Tuple[Path, MpCommonArgs]):
    ...
    prebuilt_alg, prebuilt_gen = _get_prebuild_paths(
        ...
        modified_alg, psy_file, prebuild_hash
    )
...
def _gen_prebuild_hash(x90_file: Path, mp_payload: MpCommonArgs):
    ...
    if mp_payload.transformation_script:
        trans_script_path = mp_payload.transformation_script(
            x90_file, mp_payload.config
        )
        if trans_script_path:
            trans_script_hash = file_checksum(trans_script_path).file_hash
...
def _get_prebuild_paths(prebuild_folder,
                        ...
                        psy_file,
                        prebuild_hash):
    name = f'{modified_alg.stem}.{prebuild_hash}{modified_alg.suffix}'
    prebuilt_alg = Path(prebuild_folder / name)
    prebuilt_gen = Path(prebuild_folder
                        / f'{psy_file.stem}.{prebuild_hash}{psy_file.suffix}')
...
NAME_KEYWORD = ('name' + OPT_WHITE + '=' + OPT_WHITE + STRING
                + OPT_WHITE + ',' + OPT_WHITE)
NAMED_INVOKE = ('call' + WHITE + 'invoke' + OPT_WHITE + r'\('
                + OPT_WHITE + NAME_KEYWORD)
...
def make_parsable_x90(x90_path: Path) -> Path:
    """
    Take out the leading name keyword in calls to invoke(), making temporary,
    parsable fortran from x90s.
    ...
    # keyword is followed by a comment line. I.e. The comment line doesn't
    # have an "&", so we get "call invoke(!" with no "&", which is a syntax
    # error.
    ...
    def repl(matchobj):
        ...
        # matchobj[1] contains the single group in the search pattern, which
