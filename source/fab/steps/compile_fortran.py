...
def process_file(arg: Tuple[AnalysedFortran, MpCommonArgs]) \
    ...
    *from* the prebuild folder into the module folder.
    ...
    with Timer() as timer:
        ...
        object_name = f'{analysed_file.fpath.stem}.{obj_combo_hash:x}.o'
        obj_file_prebuild = mp_common_args.config.prebuild_folder / object_name
        mod_file_prebuilds = [
        ...
        if not all(prebuilds_exist):
            ...
            #       can change less frequently than object.
            for mod_def in analysed_file.module_defs:
                hashed_module_name = f'{mod_def}.{mod_combo_hash:x}.mod'
                shutil.copy2(
                    mp_common_args.config.build_output / f'{mod_def}.mod',
                    mp_common_args.config.prebuild_folder / hashed_module_name,
                )
