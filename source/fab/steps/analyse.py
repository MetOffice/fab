...
# simple mp pattern (i.e we don't have a list of artefacts and a function to
# feed them through).
@step
def analyse(
        config,
    ...
    :param unreferenced_deps:
        ...
        automatically determined by Fab.
        ...
        those files and all their dependencies will be added to the build
        tree(s).
    ...
    # ToDo: a code smell?: we insist on the manual analysis results,
    #       special_measure_analysis_results, arriving as a list not a set
    #       because we don't want to hash them yet, because the files they
    #       refer to probably don't exist yet, because we're just creating
    #       steps at this point, so there's been no grab...
    ...
    # shall we search the results for fortran programs and a c function
    # called main?
    if find_programs:
        # find fortran programs
        sets_of_programs = [af.program_defs
                            for af in by_type(analysed_files,
                                              AnalysedFortran)]
    ...
    # add the file dependencies for MO FCM's "DEPENDS ON:" commented file
    # deps (being removed soon)
...
def _analyse_dependencies(analysed_files: Iterable[AnalysedDependent]):
    ...
    source_tree: Dict[Path, AnalysedDependent] = {a.fpath: a
...
def _extract_build_trees(root_symbols, project_source_tree, symbol_table):
    ...
    for root in root_symbols:
        ...
        logger.info(
            f"target source tree size {len(build_tree)} "
...
def _parse_files(config,
    ...
    with TimerLogger(
        ...
        fortran_results = run_mp(config, items=fortran_files,
    ...
    with TimerLogger(f"analysing {len(c_files)} preprocessed c files"):
        ...
        if sys.version.startswith('3.7'):
            warnings.warn(
                "Python 3.7 detected. Disabling multiprocessing "
                "for C analysis."
            )
            no_multiprocessing = True
        c_results = run_mp(config, items=c_files,
                           func=c_analyser.run,
    ...
    if exceptions:
        ...
        print(
            ...
            f"\n\n{err_str}\n\n",
            file=sys.stderr
        )  # ToDo: What the hell is this? Why no log?
    ...
    non_empty = {af for af in analysed_files
    ...
    if special_measure_analysis_results:
        ...
        for r in special_measure_analysis_results:
            if r.fpath in already_present:
                ...
                #       override results for files which don't *crash* the
                #       parser. We don't have a use case to do this, but it's
                #       worth noting.
                #       If we want to allow this we can raise a warning
            ...
            if symbol_def in symbols:
                duplicates.append(
                    ValueError(
                        ...
                        f"in {analysed_file.fpath} already found "
                        f"in {symbols[symbol_def]}"
                    )
                )
                continue
...
def _add_unreferenced_deps(unreferenced_deps, symbol_table: Dict[str, Path],
    ...
    for symbol_dep in unreferenced_deps:
        ...
        if analysed_file.fpath in build_tree:
            logger.info(
                f"file {analysed_file.fpath} for unreferenced "
                f"dependency {symbol_dep} is already in the build tree"
            )
            continue
