...
class DefaultLinkerSource(ArtefactsGetter):
    ...
    Looks for the default output from archiving objects, falls back to default
    compiler output. This allows a link step to work with or without a
    preceding object archive step.
...
def link_exe(config, flags=None, source: Optional[ArtefactsGetter] = None):
    ...
    from an :class:`~fab.steps.archive_objects.ArchiveObjects` step, and falls
    back to using output from compiler steps.
