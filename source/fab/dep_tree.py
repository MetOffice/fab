...
##############################################################################
"""
Classes and helper functions related to the dependency tree, as created by the
analysis stage.
"""
...
class AnalysedDependent(AnalysedFile, ABC):
    """
    An :class:`~fab.parse.AnalysedFile` which can depend on others, and be a
    dependency. Instances of this class are nodes in a source dependency tree.
...
def extract_sub_tree(
    ...
    """
    Extract the subtree required to build the target, from the full source
    tree of all analysed source files.
    ...
    _extract_sub_tree(src_tree=source_tree, key=root,
                      dst_tree=result, missing=missing, verbose=verbose)
...
def _extract_sub_tree(src_tree: Dict[Path, AnalysedDependent], key: Path,
                      ...
                      missing: Set[Path], verbose: bool, indent: int = 0):
    ...
    for file_dep in node.file_deps:
        ...
        # add this child dep
        _extract_sub_tree(src_tree=src_tree,
                          key=file_dep,
                          dst_tree=dst_tree,
                          missing=missing,
                          verbose=verbose,
...
def validate_dependencies(source_tree):
    ...
    for f in source_tree.values():
        missing.update([str(file_dep)
                        for file_dep in f.file_deps
