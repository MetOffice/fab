...
class AnalysedFile(ABC):
    ...
    def __init__(self,
        ...
        If not provided, the `self.file_hash` property is lazily evaluated in
        case the file does not yet exist.
        """
    ...
    def file_hash(self) -> int:
        if self._file_hash is None:
            if not self.fpath.exists():
                raise ValueError(
                    f"analysed file '{self.fpath}' does not exist"
                )
            self._file_hash = file_checksum(self.fpath).file_hash
        return self._file_hash

    def __eq__(self, other):
        # todo: better to use self.field_names() instead of vars(self) in
        #       order to evaluate any lazy attributes?
    ...
    def field_names(cls):
        """
        Defines the order in which we want fields to appear in str or repr
        strings.

        Calling this helps to ensure any lazy attributes are evaluated
        before use, e.g when constructing a string representation of the
        instance, or generating a hash value.
        """
        return ['fpath', 'file_hash']

    def __str__(self):
        # We use self.field_names() instead of vars(self) in order to evaluate
        # any lazy attributes.
    ...
    def __repr__(self):
        params = ', '.join([f'{f}={repr(getattr(self, f))}'
                            for f in self.field_names()])
        return f'{self.__class__.__name__}({params})'

    # We need to be hashable before we can go into a set, which is useful for
    # our subclasses. Note, the numerical result will change with each Python
    # invocation.
    def __hash__(self):
        # Build up a list of things to hash, from our attributes.
        # We use self.field_names() rather than vars(self) because we want to
        # evaluate any lazy attributes. We turn dicts and sets into sorted
        # tuples for hashing.
        # todo: There's a good reason dicts and sets aren't supposed to be
        #       hashable. Please see https://github.com/metomi/fab/issues/229
        things = set()
        for field_name in self.field_names():
...
#       We should save empty files and allow the loading to detect this, as it
#       already reads the class name.
class EmptySourceFile(AnalysedFile):
    ...
    @classmethod
    def from_dict(cls, d):
        # todo: load & save should be implemented here and used by the calling
        #       code, to save reanalysis.
        raise NotImplementedError
