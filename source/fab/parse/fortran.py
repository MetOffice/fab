...
# todo: what else should we be importing from 2008 instead of 2003? This seems
#       fragile.
...
class AnalysedFortran(AnalysedDependent):
    """
    An analysis result for a single file, containing module and symbol
    definitions and dependencies.

    The user should be unlikely to encounter this class. If the third-party
    fortran parser is unable to process a source file, a
    :class:`~fab.dep_tree.FortranParserWorkaround` object can be provided to
    the :class:`~fab.steps.analyse.Analyse` step, which will be converted at
    ...
    def __init__(self,
        ...
        :param psyclone_kernels:
            The hash of any PSyclone kernel metadata found in this source
            file, by name.
        """
        ...
        self.mo_commented_file_deps: Set[str] = set(mo_commented_file_deps
                                                    or [])
    ...
    def mod_filenames(self):
        """
        The mod_filenames property defines which module files are expected to
    ...
    @classmethod
    def field_names(cls):
        # we're not using the super class because we want to insert, not
        # append the order of our attributes
        return [
            'fpath', 'file_hash',
            'program_defs',
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result.update({
            ...
            "mo_commented_file_deps": list(sorted(self.mo_commented_file_deps)),  # noqa: E501
    ...
    def validate(self):
        ...
        assert all([d and len(d) for d in self.program_defs]), \
            "bad program definitions"
        assert all([d and len(d) for d in self.module_defs]), \
            "bad module definitions"
        assert all([d and len(d) for d in self.symbol_defs]), \
            "bad symbol definitions"
        assert all([d and len(d) for d in self.module_deps]), \
...
# todo: consider, this doesn't really need to be a class at all...it could
#       just be a function...
class FortranAnalyser(FortranAnalyserBase):
    """
    A build step which analyses a fortran file using fparser2, creating an
    ...
    def walk_nodes(self, fpath, file_hash, node_tree) -> AnalysedFortran:
        ...
        for obj in iter_content(node_tree):
            ...
            try:
                ...
                # todo: This was used for exporting a Fortran variable for
                #       use in C. Variable bindings are bidirectional - does
                #       this work the other way round, too? Make sure we have
                #       a test for it.
                ...
                # Record any psyclone kernel metadata (type definitions) we
                # find.
                # todo: how can we separate this psyclone concern out
                #       elegantly, for loose coupling?
                    ...
                    try:
                        ...
                        if type_spec.children[0] == 'EXTENDS':
                            if (
                                    ...
                                    and type_spec.children[1].string == 'kernel_type'  # noqa: E501
                            ):

                                # We've found a psyclone kernel metadata.
                                # What's it called?
                                kernel_name = _typed_child(stmt,
                                                           Type_Name).string

                                # Hash this kernel metadata.
                                # If it changes, Psyclone will reprocess any
                                # x90 which uses it.
                                kernel_hash = string_checksum(str(obj))

                                assert kernel_name not in analysed_fortran.psyclone_kernels  # noqa: E501
                                analysed_fortran.psyclone_kernels[kernel_name] = kernel_hash  # noqa: E501
                    except Exception:
                        pass
    ...
    def _process_variable_binding(self,
                                  analysed_file,
        ...
        #     logger.debug("unnamed variable binding, using fortran "
        #                  f"name '{name}' in {fpath}")
        # else:
    ...
    def _process_comment(self, analysed_file, obj):
        # Handle dependencies from Met Office "DEPENDS ON:" code comments
        # which refer to a c file. Be sure to alert the user that this
        # practice is deprecated.
        # TODO: error handling in case we catch a genuine comment
        # TODO: separate this project-specific code from the generic f
        #       analyser?
        ...
        if depends_str in obj.items[0]:
            ...
            if dep.endswith(".o"):
                analysed_file.mo_commented_file_deps.add(dep.replace(".o",
                                                                     ".c"))
    ...
    def _process_subroutine_or_function(self, analysed_file, fpath, obj):
        ...
        if bind:
            ...
            if not name:
                name = _typed_child(obj, Name)
                logger.debug(
                    f"unnamed binding, using fortran name '{name}' in {fpath}"
                )
            ...
            # exporting from fortran to c, i.e binding without an interface
            # block
            else:
        ...
        elif not _has_ancestor_type(obj, Module) \
                and not _has_ancestor_type(obj, Interface_Block):
...
class FortranParserWorkaround(object):
    ...
    This class is intended to be passed to the
    ...
    def as_analysed_fortran(self):
        # To be as helpful as possible, we allow the user to omit module
        # defs/deps from the symbol defs/deps. However, they need to be there
        # so do this now.
