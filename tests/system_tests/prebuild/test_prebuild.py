...
class TestFortranPrebuild(object):
    ...
    def build_config(self, fab_workspace, grab_prebuild_folder=None):
        # remove FFLAGS from the *mocked*, i.e copy of, the environment
        # variables
        if os.getenv('FFLAGS'):
    ...
    def test_repeatable_fmod_hashes(self, tmp_path):
        ...
        # This is achieved by changing to the source file's folder during
        # compilation. If we don't do this, then modules built in different
        # workspaces can have different folders embedded in them, thereby
        # changing their hash, thereby changing the hash of things which depend
        # on them. Build the project in "some other fab workspace", from where
        # we'll share its prebuild
        ...
        pb_hashes1 = {
            f.relative_to(
                config1.build_output
            ): zlib.crc32(open(f, 'rb').read())
            for f in pb_files1
        }

        config2 = self.build_config(
            fab_workspace=tmp_path / 'second_workspace'
        )
        pb_files2 = set(file_walk(config2.prebuild_folder))
        pb_hashes2 = {
            f.relative_to(
                config2.build_output
            ): zlib.crc32(open(f, 'rb').read())
            for f in pb_files2
        }

        # Discount the analysis results, which  will have different contents
        # because they include the source folder, which changes between
        # workspaces, but that doesn't cause a problem.
        pb_hashes1 = {p: h for p, h in pb_hashes1.items() if p.suffix != '.an'}
        pb_hashes2 = {p: h for p, h in pb_hashes2.items() if p.suffix != '.an'}

        # Make sure the remaining prebuild file contents are the same in both
        # workspaces.
    ...
    def test_vanilla_prebuild(self, tmp_path):
        ...
        # build the project in "some other fab workspace", from where we'll
        # share its prebuild
        first_project = self.build_config(
            fab_workspace=tmp_path / 'first_workspace'
        )
        ...
        first_prebuilds = {
            p.name for p in (file_walk(first_project.prebuild_folder))
        }
        second_prebuilds = {
            p.name for p in (file_walk(second_project.prebuild_folder))
        }
    ...
    def test_deleted_original(self, tmp_path):
        # Ensure we compile the files in our source folder and not those
        # specified in analysis prebuilds.
        # (We could have copied the analysis prebuilds from another
        # user/location).
        # We do this by deleting the source folder from the first build.
        # We also delete the compiler prebuilds to get the compiler to run a
        # second time.
        first_project = self.build_config(
            fab_workspace=tmp_path / 'first_workspace'
        )
        ...
        # This is not the source folder, it's the preprocessing results in the
        # build output.
