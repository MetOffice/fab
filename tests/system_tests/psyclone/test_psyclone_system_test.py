...
# this is the sanitised version, with the name keywords removed, so it is
# parsable fortran
...
# todo: Tidy up the test data in here. There are two sample projects, one not
#       even in its own subfolder.
...
def test_make_parsable_x90(tmp_path):
    # turn x90 into parsable fortran by removing the name keyword from calls
    # to invoke
    ...
    # we don't want to leave this test output in our working copies, so delete
    # it. Otherwise, it'll appear in the output from `git status`.
...
class TestX90Analyser:
    ...
    def run(self, tmp_path):
        ...
        with BuildConfig('proj', ToolBox(), fab_workspace=tmp_path) as config:
            ...
            analysed_x90, _ = x90_analyser.run(parsable_x90_path)
    ...
    def test_prebuild(self, tmp_path):
        ...
        # Run it a second time, ensure it's not re-processed and still gives
        # the correct result
...
class Test_analysis_for_x90s_and_kernels:
    ...
    def test_analyse(self, tmp_path):
        with BuildConfig('proj', fab_workspace=tmp_path,
            ...
            all_kernel_hashes = _analyse_kernels(
                config, kernel_roots=[Path(__file__).parent]
            )
...
@pytest.mark.skipif(not Psyclone().is_available,
...
class TestPsyclone:
    ...
    def steps(self, config):
        ...
        #       Perhaps add validation, warn if it's not in the build_output
        #       folder?
        psyclone(config, kernel_roots=[
            config.build_output / 'kernel',
            # this second folder is just to test the multiple folders code,
            # which was bugged. There's no kernels there.
    ...
    def test_run(self, config):
        ...
        expect_prebuild_files = [
            ...
            # The kernel hash differs between fpp and cpp, so just use
            # wildcards.
        ...
        assert all(list(config.prebuild_folder.glob(f)) == []
                   for f in expect_prebuild_files)
        assert all(list(config.build_output.glob(f)) == []
                   for f in expect_build_files)
        with config, pytest.warns(
                UserWarning,
    ...
    def test_prebuild(self, tmp_path, config):
        ...
        with mock.patch(
                'fab.parse.x90.X90Analyser.walk_nodes'
        ) as mock_x90_walk, mock.patch(
            'fab.parse.fortran.FortranAnalyser.walk_nodes'
        ) as mock_fortran_walk, mock.patch(
            'fab.tools.psyclone.Psyclone.process'
        ) as mock_run, config, pytest.warns(
            UserWarning, match="no transformation script specified"
        ):
            self.steps(config)
...
class TestTransformationScript:
    ...
    def test_transformation_script(self):
        ...
        with mock.patch('fab.tools.psyclone.Psyclone.run') as mock_run_command:
            ...
            mock_transformation_script.assert_called_once_with(
                Path(__file__), None
            )
            # check transformation_script is passed to psyclone command
            # with '-s'
