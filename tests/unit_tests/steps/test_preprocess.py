...
def test_big_little(tmp_path: Path,
    ...
    big_f90.write_text("Big F90 file.", encoding="utf-8")
    little_f90 = Path(config.source_root / 'little.f90')
    little_f90.write_text("Little f90 file.", encoding="utf-8")

    def source_getter(_: ArtefactStore) -> list[Path]:
...
def test_wrong_exe(stub_tool_repository: ToolRepository,
                   tmp_path: Path,
                   fake_process: FakeProcess) -> None:
    """
