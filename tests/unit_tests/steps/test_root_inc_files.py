...
class TestRootIncFiles(object):
    ...
    def test_vanilla(self):
        ...
        with mock.patch('fab.steps.root_inc_files.shutil') as mock_shutil:
            with mock.patch('fab.steps.root_inc_files.Path.mkdir'), pytest.warns(UserWarning, match="_metric_send_conn not set, cannot send metrics"):
    ...
    def test_name_clash(self):
        ...
        with pytest.raises(FileExistsError):
            with mock.patch('fab.steps.root_inc_files.shutil'):
                with mock.patch('fab.steps.root_inc_files.Path.mkdir'), pytest.warns(DeprecationWarning, match="RootIncFiles is deprecated as .inc files are due to be removed."):
                    root_inc_files(config)
