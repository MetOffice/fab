...
def fixture_analysed_files():
    ...
    b = AnalysedFortran(fpath=Path('b.f90'),
                        ...
                        file_hash=0)
    c = AnalysedFortran(fpath=Path('c.f90'),
                        file_hash=0)
    return a, b, c
    ...
    def test_vanilla(self, analysed_files, tool_box: ToolBox):
        ...
        with mock.patch('fab.steps.compile_fortran.run_mp',
            ...
            with mock.patch('fab.steps.compile_fortran.get_mod_hashes'):
                uncompiled_result = compile_pass(
                    config=config,
                    compiled=compiled,
...
class TestProcessFile:
    ...
    def ensure_mods_stored(self, mock_copy, mods_combo_hash):
        ...
        mock_copy.assert_has_calls(
            calls=[
                call(output_path / 'mod_def_1.mod',
                     prebuild_path / f'mod_def_1.{mods_combo_hash}.mod'),
                call(output_path / 'mod_def_2.mod',
                     prebuild_path / f'mod_def_2.{mods_combo_hash}.mod'),
            ],
            any_order=True,
        )
    ...
    def test_without_prebuild(self, content):
        ...
        with mock.patch('pathlib.Path.exists',
                        return_value=False):  # no output files exist
            with mock.patch(
    ...
    def test_compiler_hash(self, content):
        # changing the compiler must change the combo hash for the mods and
        # objects
        (mp_common_args,
         flags,
         analysed_file,
    ...
    def test_mod_missing(self, content):
        ...
        expect_object_fpath = Path(
            f'/fab/proj/build_output/_prebuild/foofile.{obj_combo_hash}.o'
        )
        assert res == CompiledFile(input_fpath=analysed_file.fpath,
                                   output_fpath=expect_object_fpath)
        mock_compile_file.assert_called_once_with(
    ...
    def test_obj_missing(self, content):
        ...
        expect_object_fpath = Path(
            f'/fab/proj/build_output/_prebuild/foofile.{obj_combo_hash}.o'
        )  # ToDo: Probably shouldn't be using absolute paths, even in testing.
        assert res == CompiledFile(input_fpath=analysed_file.fpath,
                                   output_fpath=expect_object_fpath)
        mock_compile_file.assert_called_once_with(
            analysed_file.fpath,
