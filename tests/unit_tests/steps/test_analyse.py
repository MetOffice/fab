...
class Test_gen_file_deps(object):
    ...
    def test_vanilla(self):
        ...
        analysed_files = [
            mock.Mock(
                spec=AnalysedDependent,
                fpath=my_file,
                symbol_deps={'my_func', 'dep1_mod', 'dep2'},
                file_deps=set()),
        ]

        _gen_file_deps(analysed_files=analysed_files, symbols=symbols)

        assert analysed_files[0].file_deps == {
            symbols['dep1_mod'], symbols['dep2']
        }
...
class Test_parse_files(object):
    ...
    # todo: test the correct artefacts are marked as current for the cleanup
    #       step.
...
class Test_add_manual_results(object):
    ...
    def test_vanilla(self):
        ...
        with mock.patch(
                'fab.parse.fortran.file_checksum',
                return_value=HashedFile(None, 123)
        ), pytest.warns(
            UserWarning,
            match="SPECIAL MEASURE: injecting user-defined analysis results"
        ):
        ...
        assert analysed_files == {
            AnalysedFortran(fpath=Path('foo.f'),
                            file_hash=123,
                            symbol_defs={'foo', })
        }
