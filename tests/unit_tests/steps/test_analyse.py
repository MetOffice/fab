...
class Test_parse_files(object):
    ...
    def test_exceptions(self, tmp_path):
        ...
        with mock.patch('fab.steps.run_mp', return_value=[(Exception('foo'), None)]), pytest.warns(UserWarning, match="deprecated 'DEPENDS ON:'"):
        # The warning "deprecated 'DEPENDS ON:' comment found in fortran code" is in "def _parse_files" in "source/steps/analyse.py"
...
class Test_add_manual_results(object):
    ...
    def test_vanilla(self):
        ...
        with mock.patch('fab.parse.fortran.file_checksum', return_value=HashedFile(None, 123)), pytest.warns(UserWarning, match="SPECIAL MEASURE: injecting user-defined analysis results"):
        # This warning "UserWarning: SPECIAL MEASURE: injecting user-defined analysis results" is in "def _add_manual_results" in "source/steps/analyse.py" 
